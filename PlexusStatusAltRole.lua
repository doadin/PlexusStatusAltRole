...
PlexusStatusAltRole.menuName = L["Specialization Role"]
...
PlexusStatusAltRole.options = false

local assigned_colors = setmetatable({}, {__index = function(t, k)
	local settings = PlexusStatusAltRole.db.profile.specRole
...
local function getrolecolor(role)
	local color = PlexusStatusAltRole.db.profile.specRole.colors[role]
...
local function setrolecolor(role, r, g, b, a)
	...
	PlexusStatus:SendMessage("Grid_ColorsChanged")
end

local function getrolefilter(role)
	return PlexusStatusAltRole.db.profile.specRole.filter[role] ~= false
end

local function setrolefilter(role, v)
	PlexusStatusAltRole.db.profile.specRole.filter[role] = v
	PlexusStatusAltRole:UpdateAllRoles()
end
...
local roleOptions = {
	...
	["assigned"] = {
		...
		args = {
			["useAssigned"] = {
				...
				order = 100,
				get = function() return PlexusStatusAltRole.db.profile.specRole.useAssigned end,
				set = function()
					local settings = PlexusStatusAltRole.db.profile.specRole
					settings.useAssigned = not settings.useAssigned
					if settings.enable then
						if settings.useAssigned then
							PlexusStatusAltRole:RegisterEvent("PLAYER_ROLES_ASSIGNED")
						else
							PlexusStatusAltRole:UnregisterEvent("PLAYER_ROLES_ASSIGNED")
			...
			["assignedOpacity"] = {
				...
				isPercent = true,
				order = 101,
				get = function () return PlexusStatusAltRole.db.profile.specRole.assignedOpacity end,
				set = function (_, v)
					local settings = PlexusStatusAltRole.db.profile.specRole
	...
	["hideInCombat"] = {
		...
		order = 95,
		get = function() return PlexusStatusAltRole.db.profile.specRole.hideInCombat end,
		set = function()
			local settings = PlexusStatusAltRole.db.profile.specRole
			settings.hideInCombat = not settings.hideInCombat
			if settings.enable then
				if settings.hideInCombat then
					PlexusStatusAltRole:RegisterMessage("Grid_EnteringCombat")
					PlexusStatusAltRole:RegisterMessage("Grid_LeavingCombat")
				else
					PlexusStatusAltRole:UnregisterMessage("Grid_EnteringCombat")
					PlexusStatusAltRole:UnregisterMessage("Grid_LeavingCombat")
				end
				PlexusStatusAltRole:UpdateAllRoles()
			end
		end,
...
function PlexusStatusAltRole:OnInitialize()
	self.super.OnInitialize(self)
	self:RegisterStatus("specRole", L["Specialization Role"], roleOptions, true)
end

function PlexusStatusAltRole:OnStatusEnable(status)
...
function PlexusStatusAltRole:OnStatusDisable(status)
	if status == "specRole" then
...
function PlexusStatusAltRole:Reset()
...
function PlexusStatusAltRole:Grid_EnteringCombat()
	local settings = self.db.profile.specRole
...
function PlexusStatusAltRole:Grid_LeavingCombat()
	local settings = self.db.profile.specRole
	if settings.enable and settings.hideInCombat then
		self:UpdateAllRoles()
	end
...
function PlexusStatusAltRole:PLAYER_ROLES_ASSIGNED()
	self:UpdateAllRoles()
end

function PlexusStatusAltRole:UpdateAllRoles()
	local settings = self.db.profile.specRole
	if settings.enable and (not settings.hideInCombat or not Plexus.inCombat) then
...
function PlexusStatusAltRole:UpdateRole(guid, unit, role)
	local gained
	local settings = self.db.profile.specRole
	if settings.enable and (not settings.hideInCombat or not Plexus.inCombat) then
		local assigned
